...
class DifferentialInverseKinematicsAction(ActionTerm):
	...
	def apply_actions(self):
		...
		# Get per-link gravity wrench (shape: [num_envs, num_links, 6])
		body_ids = [j + 6 for j in self._joint_ids]  # skip fixed links
		gravity_forces = self._asset.root_physx_view.get_gravity_compensation_forces()[:, body_ids]  # [B, L, 6]

		# Apply gravity compensation torques to actuated joints
		self._asset.set_joint_effort_target(gravity_forces, joint_ids=self._joint_ids)
