...
class BinaryJointPositionAction(BinaryJointAction):
	...
	def apply_actions(self):
		...
		gravity = torch.zeros(self.num_envs, 2, device=self.device) # 2dof
		body_ids = [j + 6 for j in self._joint_ids]
		gravity[:] = self._asset.root_physx_view.get_gravity_compensation_forces()[:, self._joint_ids]
		self._asset.set_joint_effort_target(gravity, joint_ids=self._joint_ids)
		# print("gravity compensation forces: ", gravity)
