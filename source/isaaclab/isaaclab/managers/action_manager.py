...
def compute_base_velocity_from_wheels_torch(wheel_velocities, wheel_radius, l, device=None):
    # same theta definition as in forward function
    theta = torch.tensor([0, 2 * torch.pi / 3, 4 * torch.pi / 3], device=wheel_velocities.device if device is None else device)

    # Construct M (same as forward)
    M = torch.stack([
        -torch.sin(theta),
        torch.cos(theta),
    ...
    # Inverse mapping
    # wheel_velocities: shape (B, 3)
    inv_matrix = torch.inverse(M.T)   # (3, 3)
    base_vel = wheel_radius * (wheel_velocities @ inv_matrix)  # Shape: (B, 3)

    # base_vel contains [vx, vy, wz]
    return base_vel
	...
	def action_absolute(self) -> torch.Tensor:
		...
		for name, term in self._terms.items():
			...
			if name in ['armR_action', 'armL_action']:
				action_abs.append(term._ik_controller.ee_pos_des) # 3
			...
			# Gripper
				...
				action_abs.append(term.processed_actions[:, 0:1]) # 1
			# Mobile base
			elif name in ['base_action']:
				action_abs.append(term.processed_actions[:, :3]) # 3

		action_abs = torch.cat(action_abs, dim=-1)	# or dim=0 if they're 1D
		action_dict = {
						"ee_6D_pos": action_abs[:, : -3],
						"base": compute_base_velocity_from_wheels_torch( action_abs[:,-3:], wheel_radius=0.103, l=0.05),
					}
		return action_dict
