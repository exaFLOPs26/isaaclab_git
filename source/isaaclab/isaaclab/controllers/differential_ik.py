...
class DifferentialIKController:
	...
	def reset(self, env_ids: torch.Tensor = None):
		...
		if env_ids is None:
			...
			self.ee_pos_des[env_ids] = torch.zeros(len(env_ids), 3, device=self._device)
			self.ee_quat_des[env_ids] = torch.zeros(len(env_ids), 4, device=self._device)
	def set_command(
		...
		if self.cfg.command_type == "position":
			...
			if self.cfg.use_relative_mode:
				...
				if torch.all(self.ee_pos_des == 0.0):
					self.ee_pos_des = ee_pos.clone()
					self.ee_quat_des = ee_quat.clone()
				self.ee_pos_des, self.ee_quat_des = apply_delta_pose(self.ee_pos_des, self.ee_quat_des, self._command)
			else:
	...
	def compute(
		...
		delta_joint_pos[delta_joint_pos.abs() < 1e-2] = 0.0
